...
import { pingRedisOnce, waitForRedis } from './wait-for-redis';

/**
 * CP-016 — regression tests for the bounded Redis reachability check that
 * gates every Redis-dependent service's own bootstrap (see main.ts and
 ...
 *     module opens — a real listener answering the real protocol
 *     exchange, not a stand-in for the function under test.
 * Real Redis integration is separately proven in
 * test/redis-reliability.e2e-spec.ts against the actual service the app
 * boots against; these tests exist to pin the bounded-retry/backoff
 * behavior deterministically and fast, without needing a live Redis.
 */
describe('waitForRedis / pingRedisOnce', () => {
  ...
  describe('pingRedisOnce (single attempt, no retry — used by the readiness endpoint)', () => {
    it('resolves once a real socket answers a real PING with +PONG', async () => {
    ...
    it('rejects immediately (no retries) against a real closed port', async () => {
      await expect(pingRedisOnce(UNREACHABLE_URL)).rejects.toThrow(/ECONNREFUSED/);
    });
  });
  ...
  describe('waitForRedis (bounded retry — used at process startup)', () => {
    ...
    it('resolves on the very first attempt once Redis is reachable', async () => {
      ...
      try {
        ...
        // No backoff should have been needed at all.
