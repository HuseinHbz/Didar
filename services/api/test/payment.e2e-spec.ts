 ...
 * `ReconciliationService` — runs its real code, unmodified), and only
 * the HTTP call to ZarinPal itself is swapped for a deterministic,
 * per-authority-configurable result. `ZarinpalAdapter`'s own request/
 * response shapes were verified separately against the real (blocked)
 * network boundary during this phase's own implementation.
 */
...
class FakePaymentProviderAdapterRegistry implements PaymentProviderAdapterRegistry {
