 ...
 * This worker's only job is running BullMQ processors (`BullModule.
 * forRootAsync` in app.module.ts) — it has required a reachable Redis to
 * boot since its own inception, and being a non-HTTP application context
 * (see main.ts's own doc comment), it has no health endpoint at all to
 * expose a stuck boot through, which makes the hang this check closes
 * even more invisible than services/api's own version of the same gap —
 * see docs/architecture/redis-reliability.md for the full account
 * (including the live reproduction against services/api that motivated
 * this fix across all three Redis-dependent services).
 *
