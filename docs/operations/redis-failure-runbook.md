...
| ----------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `services/api` won't start; logs show 5x `Redis unreachable at <host>:<port>` then exits                                            | Redis unreachable at boot                                                                                                                                         | `GET /health/ready` on a _running_ instance (if one is still up) confirms; check the Redis host/port resolved from `REDIS_URL` is correct and reachable from the network the service runs on                                                                                                         |
...
2. **Do not restart `services/api` instances that are already running**
   ...
   _out_ of rotation for no benefit.
