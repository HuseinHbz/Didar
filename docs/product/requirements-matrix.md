...
| REQ-PROC-01            | Admin can create and receive Purchase Orders against a Supplier    | Admin     | Receiving writes to the existing `InventoryLedger`, concurrency-safe              | `inventory.purchase_order.*`/`inventory.supplier.manage` RBAC | PO lifecycle audited (`PURCHASE_ORDER_CREATED`/`_APPROVED`/`_RECEIVED`/`_CANCELLED`) | Concurrency e2e — sequential retry + 20-way-concurrent idempotent receive (same bar as CP-006) | DONE (CP-021)    |
...
| REQ-PLAT-02 | A Redis outage fails fast and legibly instead of hanging indefinitely | System/Operator | Killing Redis produces a bounded, logged failure within a documented timeout, not an infinite retry loop | N/A                                    | Failure logged               | Regression test re-running this audit's own empirical reproduction | PLANNED (CP-016) |
| REQ-PLAT-03 | Public routes are protected against brute-force/abuse                 | Customer/System | A scripted burst against OTP/login is rejected past a documented threshold                               | Additive, does not weaken RBAC/JWT     | Rate-limit rejections logged | New unit/e2e test                                                  | PLANNED (CP-016) |
