...
| ID           | Business Value                                                          | User Type | Acceptance Criteria                                                   | Security                                    | Audit                  | Test                                            | Status                                                                                                                                                                                                                                  |
| ------------ | ----------------------------------------------------------------------- | --------- | --------------------------------------------------------------------- | ------------------------------------------- | ---------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| REQ-NOTIF-01 | OTP/order-confirmation SMS actually reaches a real Iranian phone number | Customer  | Real send confirmed in a staging environment with real network egress | Provider credentials never logged/committed | Delivery status logged | Contract test against `NotificationChannelPort` | **PARTIAL (CP-017)** — real Kavenegar adapter implemented and unit-tested against a real local HTTP server; live network egress unverified from this sandbox (proxy denial, same as REQ-PAY-02's own ZarinPal precedent) — see gap P1-8 |
