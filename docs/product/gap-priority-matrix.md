...
| ---- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1-1 | No rate limiting anywhere in `services/api`                                                                   | Grepped, zero `ThrottlerModule`, audit §8 / risk R3                                              | **Unassigned, still explicitly deferred** — CP-017 added a narrow, non-exhaustive `(phone, purpose)`-keyed SMS-dispatch cooldown on `POST /auth/otp/request` only (see ADR-014); every other endpoint remains unthrottled  |
| P1-2 | Every notification-channel adapter is a stub — OTP/order-confirmation delivery is fake in any real deployment | Adapter source, self-labeled `⚠️ Stub`, audit §9 / risk R4                                       | **RESOLVED for SMS by CP-017** — real Kavenegar integration, see `docs/adr/ADR-014-real-notification-delivery.md`. Telegram/WhatsApp/Email/Push remain stubs, explicitly out of CP-017's own scope — no owner assigned yet |
...
| P1-8 | Live Kavenegar SMS gateway network path never verified                                                        | Live proxy-denial confirmed in CP-017 (same technique as P1-6), ADR-014                          | **CP-017 remainder** (staging verification task, not new code)                                                                                                                                                             |

## P2 — enhancements

| ID    | Gap                                                                                                                         | Evidence                                           | Owner phase                                                                                      |
| ----- | --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| P2-1  | No Purchase Order / Supplier model                                                                                          | Blueprint PHASE 4 bullets absent from schema/code  | **CP-021**                                                                                       |
